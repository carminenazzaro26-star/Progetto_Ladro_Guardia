...
class MinimaxGuardAI:
    ...
    def can_see(self, grid, g1, robber_pos):
        ...
        # Se robber_pos è None (ladro non visibile o lontano nel test), ritorniamo False
        if robber_pos is None:
            return False

        # Se il ladro esiste, calcoliamo la distanza
        if self.visual_range >= g1.manhattan(robber_pos):
            return True
        return False

    def has_line_of_sight(self, grid, start: Position, end: Position):
        if end is None: return False  # Protezione aggiuntiva
    ...
    def get_best_moves(self, state: GameState):
        # Qui usiamo la can_see corretta che gestisce il None
