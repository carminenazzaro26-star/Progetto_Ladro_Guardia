...
class MinimaxGuardAI:
    def __init__(self, max_depth=2, visual_range=4):
        ...
        self.visual_range = visual_range  # Questo parametro ora è variabile!
        self.moves = [(0, 1), (0, -1), (1, 0), (-1, 0), (0, 0)]
        self.last_known_pos = None

    def can_see(self, grid, g1, robber_pos):
        # --- FIX IMPORTANTE ---
        # Se il ladro non esiste (è None), non posso vederlo!
        if robber_pos is None:
            return False

        # Controllo distanza usando il raggio visivo corrente
    ...
    def has_line_of_sight(self, grid, start: Position, end: Position):
        ...
        while (temp_x != target_x or temp_y != target_y):
            ...
            if 0 <= temp_y < len(grid) and 0 <= temp_x < len(grid[0]):
    ...
    def get_best_moves(self, state: GameState):
        # 1. Controllo visibilità con il nuovo metodo sicuro
        ...
        if visible:
            ...
            # Se siamo arrivati sull'ultima posizione nota e non c'è nessuno, resettiamo
        ...
        for g1 in m1_list:
            ...
            for g2 in m2_list:
                ...
                # Se stiamo inseguendo un fantasma (memoria), profondità 0 per non perdere tempo
